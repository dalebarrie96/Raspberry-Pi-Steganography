...
function firstEncryptMessageTest(){
	...
	fs.readFile("documents/message.txt", 'utf8', function(err, data) {
	  ...
	  for (var i = 0; i < data.length; i++) {
		_binaryString += byteBinary(data[i].charCodeAt(0).toString(2));
	...
	fs.readFile('output.png', function(err, buffer){
		...
		Jimp.read(buffer)
			.then(image => {
				...
				console.log(binaryStringToText(binaryString));
...
function binaryStringToText(binary){
	console.log(binary);
